...
<form action="/asc/project/deleteProject" method="get">
...
<table>
	...
	<tr>
		...
		<td>
			<input type="text" value="${project.projectName}" id="projectName" readOnly="true">
		</td>
		<c:if test="${project.teamLeaderSeq eq member.userSeq && project.finishYn ne 'S'}">
			<td class="tableName">
				<input type="button" value="수정" id="clickModify" style="">
			</td>
		</c:if>
	</tr>
	<tr>
		...
		<td>
			<input type="text" value="${project.finishYn}" readOnly="true">
		</td>
	</tr>
	<tr>
		<td class="tableName">
...
<c:if test="${project.finishYn ne 'S'}">
	<input type="submit" value="중단" onclick="return confirm('정말로 프로젝트를 중단하시겠습니까 ?');" style="margin-top: 8px;margin-left: 5px;">
</c:if>
</form>
</div>
</body>
